 ...
 * @date 2021/12/21
 */
public interface MoodstoreUserDetailsService extends UserDetailsService, Ordered {
	...
	default UserDetails getUserDetails(R<UserInfo> result) {
		...
		// 构造security用户
		return new MoodstoreUser(user.getUserId(), user.getDeptId(), user.getUsername(),
	 ...
	 * @return
	 */
	default UserDetails loadUserByUser(MoodstoreUser moodstoreUser) {
		return this.loadUserByUsername(moodstoreUser.getUsername());
	}
