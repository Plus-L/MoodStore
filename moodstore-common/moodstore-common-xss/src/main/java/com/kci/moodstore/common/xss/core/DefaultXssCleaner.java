...
package com.kci.moodstore.common.xss.core;

import cn.hutool.core.util.CharsetUtil;
import com.kci.moodstore.common.xss.config.MoodstoreXssProperties;
import com.kci.moodstore.common.xss.utils.XssUtil;
...
public class DefaultXssCleaner implements XssCleaner {
	...
	private final MoodstoreXssProperties properties;

	public DefaultXssCleaner(MoodstoreXssProperties properties) {
		this.properties = properties;
	}

	private static Document.OutputSettings getOutputSettings(MoodstoreXssProperties properties) {
	...
	public String clean(String bodyHtml, XssType type) {
		...
		MoodstoreXssProperties.Mode mode = properties.getMode();
		if (MoodstoreXssProperties.Mode.escape == mode) {
			// html 转义
			return HtmlUtils.htmlEscape(bodyHtml, CharsetUtil.UTF_8);
		}
		else if (MoodstoreXssProperties.Mode.validate == mode) {
			// 校验
